...
sealed trait Json {
  /**
   * Try to decode this JSON value into a value of type `A` by using the implicit decoder.
   *
...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromFunction {
      case Json.Num(x) if x.isValidInt => Option(x.toInt)
      case _ => None
    }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction { case Json.Arr(items: List[Json]) =>
      ...
      if (array.contains(None)) None
      else Option(array.flatten)
    }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction { case Json.Obj(fields) =>
      decoder.decode(fields.getOrElse(name, Json.Null))
    }
}
